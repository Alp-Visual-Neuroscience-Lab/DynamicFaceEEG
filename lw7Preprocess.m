...
ids = [1:25]; % input ids
baseFilePath = 'C:\Users\eeglab1\Desktop\Yasemin\FaceTempOrd'; % input baseFilePath
intFile = fullfile(baseFilePath, 'Interpolation.xlsx');

desiredEvents = {'S101','S102','S103','S104','S201','S202','S203','S204'};% input event codes


ppFilePath = fullfile(baseFilePath, 'Preprocessed');
mkdir(ppFilePath);
cd(ppFilePath)
...
for id = ids
    ...
    filePath = [baseFilePath, '\RAW\P', num2str(id), '\'];   %each participants .eeg file should be in a root folder called RAW/P{id}
    ...
    for i = 1:length(files)
        ...
        option=struct('filename',fileName);
        lwdata= FLW_load.get_lwdata(option);
        fileName = interpolate(lwdata, id, intFile);
